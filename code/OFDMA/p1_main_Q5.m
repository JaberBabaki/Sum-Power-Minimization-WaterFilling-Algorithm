...
for j=1:15
    ...
    for o=1:5
        ...
        pathGain=FuncPowerPlus(pathGain,RminPower2,C(o),PowerMax);
        for p=1:C(o)
            if(pathGain(p,3)> PowerMax)             
                for h=1:10
                    if(pathGain(h,3)==0)
                      pathGain(h,3)=1;  
                    end
                end
                pathGain(p,3)=PowerMax;
                Rmin=Rmin-log2(1+pathGain(p,3)*pathGain(p,2));
                RminPower2=2^(Rmin); 
                pathGain=FuncPowerPlus(pathGain,RminPower2,C(o),PowerMax);
            end
        end
